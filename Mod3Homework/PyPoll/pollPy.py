...
# print()
# print("----------------------------------------")
#changing to write to .txt
outFile = os.path.join("Analysis","result.txt")
with open(outFile,"w",newline='') as output:
    writeRes = csv.writer(output)
    writeRes.writerow([f"Election Results"])
    writeRes.writerow([])
    writeRes.writerow(["----------------------------------------"])
    writeRes.writerow([])
    writeRes.writerow([f"Total Votes: {len(idB)}"])
    writeRes.writerow([])
    writeRes.writerow(["----------------------------------------"])
    writeRes.writerow([])
    writeRes.writerow([f"Charles Casper Stockham: {round(ccsVote/len(idB)*100,3)}% ({ccsVote})"])
    writeRes.writerow([f"Diana DeGette: {round(ddgVote/len(idB)*100,3)}% ({ddgVote})"])
    writeRes.writerow([f"Raymon Anthony Doane: {round(radVote/len(idB)*100,3)}% ({radVote})"])
    writeRes.writerow([])
    writeRes.writerow(["----------------------------------------"])
    writeRes.writerow([])
    writeRes.writerow([f"Winner: {winner}"])
    writeRes.writerow([])
    writeRes.writerow(["----------------------------------------"])