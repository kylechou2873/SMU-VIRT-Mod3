...
with open(pathFile) as csvFile:
    ...
    #changing to writing in .txt
    outFile = os.path.join("Analysis","result.txt")
    with open(outFile, "w", newline='') as output:
        writeRes = csv.writer(output)
        #write header
        writeRes.writerow(["Financial Analysis"])
        writeRes.writerow(["-----------------------------"])
        writeRes.writerow([f"Total Months: {len(dateD)}"])
        writeRes.writerow([f"Total: ${sum(proLosD)}"])
        writeRes.writerow([f"Average Change: ${round(sum(netList)/len(netList),2)}"])
        writeRes.writerow([f"Greatest Increase in Profit: {dateInc} (${proInc})"])
        writeRes.writerow([f"Greatest Decrease in Profit: {dateDec} (${proDec})"])